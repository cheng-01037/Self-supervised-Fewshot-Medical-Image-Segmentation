...
def cfg():
    ...
    if mode == 'train':
        ...
        ignore_label = 255
        print_interval = 100
        validation_interval = 25000 # validation after * iterations
        ...
        save_interm_predicts = True # save intermediate prediction for visual debugging
        z_margin = 0 
        ...
        clsname = None # 
        use_wce = True
        reload_model_path = None # path for reloading a trained model, other than mscoco initialization
    ...
    exp_str = '_'.join(
        [exp_prefix]
        + [dataset,]
        + [f'sets_{label_sets}_{task["n_shots"]}shot_{mode}'])
